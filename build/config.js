...
var Config = function Config(options) {
  ...
  var enabled = coalesce(options.enabled, process.env.DD_TRACE_ENABLED, true);
  var debug = coalesce(options.debug, process.env.DD_TRACE_DEBUG, false);
  var protocol = 'http';
  var hostname = coalesce(options.hostname, process.env.DD_TRACE_AGENT_HOSTNAME, 'localhost');
  var port = coalesce(options.port, process.env.DD_TRACE_AGENT_PORT, 8126);
